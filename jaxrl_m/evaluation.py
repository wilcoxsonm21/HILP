...
from hilp.special_networks import concatenate, stack
...
def add_episode_info(env_name, env, info, trajectory):
    if "antmaze" in env_name:
        if (
            type(trajectory["next_observation"][-1]) == dict
            or type(trajectory["next_observation"][-1]) == FrozenDict
        ):
...
def evaluate_with_trajectories(
    ...
    for i in trange(num_episodes + num_video_episodes):
        ...
        observation, obs_goal = env_reset(
            env_name, env, goal_info, base_observation, policy_type, vam_info=vam_info
        )
        done = False
        ...
        while not done:
            ...
            if policy_type == "goal_skill":
                ...
                phi_obs, phi_goal = agent.get_phi(
                    stack((policy_obs, policy_goal), axis=0)
                )
