...
from hilp.special_networks import (GoalConditionedCritic,
                                   GoalConditionedPhiValue,
                                   GoalConditionedValue, HILPNetwork)
...
def loss_fn(network_params, agent, batch):
    ...
    batch["skills"] = random_skills / jnp.linalg.norm(
...
class HILPAgent(flax.struct.PyTreeNode):
    ...
    def update(agent, batch):
        ...
        new_skill_target_params = jax.tree_map(
            lambda p, tp: p * agent.config["target_update_rate"]
            + tp * (1 - agent.config["target_update_rate"]),
            agent.network.params["networks_skill_critic"],
            agent.network.params["networks_skill_target_critic"],
        )
    ...
    def sample_skill_actions(
        ...
        *,
        ...
        dist = agent.network(
            observations, skills, temperature=temperature, method="skill_actor"
        )
...
def create_learner(
    ...
    **kwargs,
    ...
    network_params = network_def.init(
        value_key, observations, observations, actions, np.zeros((1, skill_dim))
    )["params"]
